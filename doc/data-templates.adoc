...
=== Extension with Requirements

Extensions can specify parameter requirements:
...
description: |
  Requires stvec.MODE to support vectored interrupts.
type: privileged
versions:
  - version: "1.0.0"
    state: ratified
...
* Instructions: `spec/std/isa/inst/I/addi.yaml`, `spec/std/isa/inst/A/lr.w.yaml`
